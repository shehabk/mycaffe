...
int adaBoostBinary::allocateMemory(int posnum, int negnum, int iterNum)
{
	...
	{
		printf("The value of pos num, neg num and iteration number should not be zero\n");
		return 0;
	}
 ...
 *@param ib The begining row position
 *@param id The ending row postion
 *@param ipos the current ipos sample number
 *@param ineg the current ineg sample number
 */
int adaBoostBinary::LoadDataAllFeat_set(CvMat *feature_train, const char* trainfile,int ib, int id,int dim,int &inum)
{
	...
	for(int j=ib; j < id; j++)
	{
		...
		{
			printf("ERROR: the train data in the file %s is more than the designed size %d\n", trainfile, inum);
			return -1;
		}
		inum++;
