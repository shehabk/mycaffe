...
void BoostingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	this->blobs_[0].reset(new Blob<Dtype>(para_shape));
	this->blobs_[1].reset(new Blob<Dtype>(para_shape));
...
void BoostingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
	...
	weak_shape[1]=N_;
	top[1]->Reshape(weak_shape);//weak score
	top[2]->Reshape(weak_shape);//weak label, actually it is the repeat of label
...
void BoostingLayer<Dtype>::adaboost(const Dtype* bottom_data, const Dtype* bottom_label, Dtype* alpha, Dtype* thresh, Dtype* sign)
{
	...
	for(int i=0; i< N_; i++)
	{
		...
		adaboost.choosefeature_fastupdate(feature_train,labelMat, &current_best_classifier,maplabel, sign,thresh,imith_,i);
		adaboost.updataWeight(feature_train, &current_best_classifier,imith_, i);
		sum_alpha+=current_best_classifier.alpha;
		errorrate_ith_iter = adaboost.testStrongClassifier();
